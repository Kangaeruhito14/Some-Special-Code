...
void compute_mandelbrot(vector<int> rows) {
    for (auto y : rows) {
        for (int x = 0; x < width; ++x) {
            ...
            if (n != MAX_ITERATIONS) {
                matrix[x][y][0] = fabs(smooth1 / DIVIDER); // Red
                matrix[x][y][1] = (smooth3 / DIVIDER); // Green
                matrix[x][y][2] = fabs(smooth2 / DIVIDER); // Blue
            } else {
                matrix[x][y][0] = 0.0; // Black
                matrix[x][y][1] = 0.0;
                matrix[x][y][2] = 0.0;
            }
        }
        ...
        if (N_CLICKS >= 2) {
            if (N_CLICKS == 2) {
                ...
                if ((duration <= 500) && (not FLAG)) {
                    ...
                    long double dx = rightcd - leftcd;
                    long double dy = top - bottom;

                    long double new_center_x = leftcd + (x * dx / width);
                    ...
                    leftcd = new_center_x - (new_delta_x / 2.0);
                    rightcd = new_center_x + (new_delta_x / 2.0);
                    bottom = new_center_y - (new_delta_y / 2.0);
                    top = new_center_y + (new_delta_y / 2.0);

                    cout << " [DEBUG] New borders: " << leftcd << " " << rightcd << " " << bottom << " " << top << endl;
